...
// DndProvider is already provided at the app level - no need to import here
...
function PlaceHolderSign() {
  ...
  const [shouldCreateFinalPdf, setShouldCreateFinalPdf] = useState(false);
  ...
  const fetchTenantDetails_old = async () => {
  ...
  const fetchTenantDetails = async () => {
    try {
      // If you track current user elsewhere, pass it here; otherwise omit.
      const tenantDetails = await getTenantDetails(/* userId */ undefined);
      if (tenantDetails && tenantDetails === "user does not exist!") {
        alert(t("user-not-exist"));
        return [];
      }
      if (tenantDetails) {
        if (tenantDetails?.RequestBody && tenantDetails?.RequestSubject) {
          setRequestBody(tenantDetails.RequestBody);
          setRequestSubject(tenantDetails.RequestSubject);
          setTenantMailTemplate({
            body: tenantDetails.RequestBody,
        ...
        const signatureType = tenantDetails?.SignatureType || [];
        const enabled = signatureType?.filter((x) => x.enabled === true);
        return enabled;
      }
      return [];
    } catch (e) {
  ...
  const getDocumentDetails = async () => {
    ...
    if (documentData && documentData.length > 0) {
      ...
      // Ensure distinct colors for each non-prefill placeholder at load time
      if (
        documentData[0]?.Placeholders &&
        documentData[0].Placeholders.length > 0
      ) {
        let colorIdx = 0;
        const normalizedPlaceholders = documentData[0].Placeholders.map(
          (ph) => {
            if (ph?.Role === "prefill") return ph;
            // Always assign new color from index to ensure uniqueness
            const assignedColor = color[colorIdx % color.length];
            colorIdx += 1;
            return { ...ph, blockColor: assignedColor };
          }
        );
        documentData[0].Placeholders = normalizedPlaceholders;
      }
  ...
  //embed prefill label widget data - only create final PDF when sending document
  const embedPrefilllData = async () => {
    ...
    if (prefillExist && prefillExist.length > 0) {
      ...
      try {
        ...
        // Save in organized structure: /uploads/{documentId}/versions/
        const pdfUrl = await saveOrganizedPdfFile(
          documentId,
          pdfName,
          pdfBase64,
          "temp"
        ...
        console.log("error to saveOrganizedPdfFile in placeholder flow", err);
        alert(err?.message);
      }
  ...
  const autosavedetails_old = async () => {
  ...
  const autosavedetails = async () => {
    const signers = signersdata?.reduce((acc, x) => {
      if (x.objectId) acc.push({ objectId: x.objectId });
      return acc;
    }, []);

    // Don't create PDF files during autosave - only save metadata
    // PDF creation should only happen when actually sending the document
    try {
      const body = {
        ...(signerPos?.length > 0 ? { Placeholders: signerPos } : {}),
        Signers: signers,
        SignatureType: signatureType,
        // Only set status to UNDER_PROCESSING if document is not already in a final state
        ...(pdfDetails?.[0]?.Status !== "PENDING_SIGNATURES" &&
        pdfDetails?.[0]?.Status !== "COMPLETED" &&
        pdfDetails?.[0]?.Status !== "DECLINED"
          ? { Status: "UNDER_PROCESSING" }
          : {}),
      };
      await fetch(`/api/opensign/documents/${documentId}`, {
        method: "PUT",
  ...
  const saveDocumentDetails_old = async () => {
    setIsUiLoading(true);
  ...
  const saveDocumentDetails = async () => {
    setIsUiLoading(true);
    setShouldCreateFinalPdf(true); // Flag to indicate we're creating the final PDF

    let signerMail = signersdata.slice();
    if (pdfDetails?.[0]?.SendinOrder === true) {
      signerMail.splice(1);
    }

    // Only create final PDF when actually sending the document
    const pdfUrl = await embedPrefilllData();
    if (pdfUrl) {
      const signers = signersdata?.map((x) => ({ objectId: x.objectId }));
      const addExtraDays = pdfDetails?.[0]?.TimeToCompleteDays || 15;
      const currentUser = signersdata.find((x) => x.Email === currentId);
      setIsCurrUser(
        pdfDetails?.[0]?.SendinOrder
          ? signerMail?.[0]?.Email === currentId
      ...
      try {
        const data = {
          ...
          SentToOthers: true,
          SignatureType: pdfDetails?.[0]?.SignatureType,
          ExpiryDate: updateExpiryDate.toISOString(),
          Status: "PENDING_SIGNATURES",
        };
        console.log("Saving document details: ", data);
        await fetch(`/api/opensign/documents/${documentId}`, {
          method: "PUT",
          headers: { "content-type": "application/json" },
        ...
        // Update local state with new status and title
        const updatedPdfDetails = [...pdfDetails];
        updatedPdfDetails[0].Status = "PENDING_SIGNATURES";
        if (docTitle) {
          updatedPdfDetails[0].Name = docTitle;
        }
        setPdfDetails(updatedPdfDetails);
    ...
    setShouldCreateFinalPdf(false); // Reset the flag
  };
  ...
  const handleShareList = () => {
    console.log("handleShareList called");
    console.log("Current pdfDetails state:", pdfDetails);
    console.log("Current signersdata:", signersdata);
    ...
    for (let i = 0; i < signerMail.length; i++) {
      ...
      // console.log("pdfDetails[0].objectId:", pdfDetails?.[0]?.objectId);
      // console.log("signerMail[i].Email:", signerMail[i].Email);
      ...
      let signPdf = `${hostUrl}/opensign/sign/${pdfDetails?.[0].objectId}?data=${encodeBase64}`;

      console.log("Generated signPdf URL:", signPdf);
  ...
  const sendEmailToSigners_old = async () => {
    let htmlReqBody;
    setIsUiLoading(true);
    setIsSendAlert({});
    let sendMail;
  ...
  const sendEmailToSigners = async () => {
    ...
    for (let i = 0; i < signerMail.length; i++) {
      try {
        ...
        sendMailResp = await fetch("/api/opensign/mail/send", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({
            extUserId: owner?.objectId,
    ...
    const status = sendMailResp && sendMailResp.ok ? "success" : "failed";
    if (status === "success") {
      setMailStatus("success");
      try {
        let data;
  ...
  const handleNameModal = (shouldOpen = false) => {
    if (shouldOpen) {
      setIsNameModal(true);
    } else {
  ...
  const closeTour = async () => {
    setPlaceholderTour(false);
    if (isDontShow && signerUserId) {
      let updatedTourStatus = [];
      if (tourStatus.length > 0) {
        updatedTourStatus = [...tourStatus];
        const idx = tourStatus.findIndex(
          (obj) => obj["placeholder"] === false || obj["placeholder"] === true
        );
        if (idx !== -1) {
          updatedTourStatus[idx] = { placeholder: true };
        } else {
          updatedTourStatus.push({ placeholder: true });
        }
      } else {
      ...
      try {
        await fetch(`/api/opensign/users/${signerUserId}`, {
          method: "PUT",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ TourStatus: updatedTourStatus }),
        });
      } catch (err) {
        console.log("axois err ", err);
      }
  ...
  const handleRecipientSign = () => {
    console.log("handleRecipientSign called with currentId:", currentId);
    console.log("documentId:", documentId);

    // Ensure we pass contactId as the assigned signer's objectId (contactbook id)
    // PdfRequestFiles expects contactId to be the signer's objectId to enable placeholders
    let contactIdParam = "";
    if (currentId && currentId.trim() !== "") {
      const selectedSigner = signersdata.find(
        (s) => s?.objectId === currentId || s?.Email === currentId
      );
      contactIdParam = selectedSigner?.objectId || "";
    }

    if (contactIdParam) {
      const url = `/opensign/sign/${documentId}?contactId=${encodeURIComponent(
        contactIdParam
      )}`;
      console.log("Navigating with contactId (objectId):", contactIdParam);
      console.log("Full URL:", url);
      window.location.href = url;
    } else {
      console.log("Navigating without contactId (no signer match found)");
      window.location.href = `/opensign/sign/${documentId}`;
    }
  };
  ...
  const loadOptions = async (inputValue) => {
    try {
      const res = await fetch("/api/opensign/signers/search", {
        method: "POST",
        headers: { "content-type": "application/json" },
      ...
      if (!res.ok) return [];
      const result = await res.json(); // [{ objectId, Name, Email }]
      setUserList(result);
  ...
  return (
    <>
      ...
      {/* DndProvider is already provided at the app level */}
      {isLoading.isLoad ? (
            ...
            <div className=" w-full md:w-[57%] flex mr-4">
              ...
              <div className=" w-full md:w-[95%] ">
                ...
                <ModalUi
                  ...
                  <AddContact details={handleAddUser} closePopup={closePopup} />
                </ModalUi>
                <PlaceholderCopy
      ...
      />
      {/* DndProvider closing tag removed - handled at app level */}
    </>
  );
}
