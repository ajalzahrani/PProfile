...
const AddSignerModal = ({ isOpen, children }) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-center justify-center backdrop-blur-sm">
      <div className="bg-white rounded-lg shadow-2xl border border-gray-200 p-0 min-w-[90%] md:min-w-[500px] max-h-[90vh] overflow-y-auto text-sm">
        {children}
      </div>
    </div>,
    document.body
    ...
    <div className="text-xs mt-2 ">
      ...
      <div className="flex gap-x-[5px]">
        ...
        <div
          ...
          className="cursor-pointer border border-gray-300 rounded-md focus:outline-none hover:border-blue-500 hover:bg-blue-50 max-h-[38px] min-w-[48px] flex justify-center items-center transition-colors duration-200">
          <i className="fa-solid fa-plus text-gray-600 hover:text-blue-600"></i>
        </div>
        <AddSignerModal isOpen={modalIsOpen}>
          <h3 className="text-gray-800 font-bold text-lg pt-4 px-5 pb-2 border-b border-gray-200">
            {t("add-contact")}
          </h3>
          <button
            onClick={handleModalCloseClick}
            className="absolute right-4 top-4 w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 hover:bg-gray-200 text-gray-600 hover:text-gray-800 transition-colors duration-200">
            ✕
          </button>
          {isModal && (
            <AddContact
