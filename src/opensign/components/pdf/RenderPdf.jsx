...
// use the worker shipped in /public
...
function RenderPdf(props) {
  ...
  const isDataUrl = (s) =>
    typeof s === "string" && s.startsWith("data:application/pdf;base64,");
  const isProbablyBase64 = (s) =>
    typeof s === "string" && /^[A-Za-z0-9+/=\s]+$/.test(s);

  let fileProp = undefined;
  const src = props.pdfBase64Url || "";
  if (isDataUrl(src)) fileProp = src;
  else if (isProbablyBase64(src))
    fileProp = `data:application/pdf;base64,${src}`;
  else fileProp = src; // assume it's a URL
        ...
        <div
          ...
          className={
            isMobile
              ? `${
                  isGuestSigner ? "30px" : ""
                } border-[0.1px] border-[#ebe8e8] overflow-x-auto relative`
              : "relative"
          }
          style={{
            width:
          ...
          <Document
            ...
            onLoadError={(e) => {
              ...
              if (typeof props.setPdfLoad === "function")
            ...
            onLoadSuccess={(pdf) => {
              ...
              if (typeof props.pageDetails === "function")
