...
// Dynamic import for print-js to avoid SSR issues
// import printModule from "print-js";
// Removed fontkit import - no longer needed since we use built-in fonts
...
// Removed fileasbytes function - no longer needed since we use built-in fonts
...
export const getSecureUrl_old = async (url) => {
...
export const getSecureUrl = async (url) => ({ url });
...
export const contractUsers_old = async () => {
  try {
...
export const contractUsers = async () => {
  try {
    const res = await fetch("/api/opensign/users/me", { cache: "no-store" });
    if (!res.ok) return "Error: Something went wrong!";
    const me = await res.json(); // { objectId, TourStatus }
    return me ? [me] : [];
  } catch (err) {
    console.log("Err in users/me", err);
    return "Error: Something went wrong!";
  }
};
...
export const getWidgetType = (item, widgetName) => {
  return (
    <div className="border border-blue-500 bg-blue-50 hover:bg-blue-100 w-fit md:w-[100%] focus:outline-none focus:ring-2 focus:ring-blue-500 ml-[6px] md:ml-0 p-1 overflow-hidden rounded-md transition-colors duration-200">
      <div className="w-full h-full flex md:justify-between items-center">
        <div className="flex justify-start items-center text-[13px] ml-1">
          {!isMobile && (
            <i className="fa-solid fa-grip-vertical ml-[3px] text-gray-600"></i>
          )}
          <span className="md:inline-block text-center text-[15px] ml-[5px] font-semibold pr-1 md:pr-0 text-gray-800">
            {widgetName}
          </span>
        </div>
        <div className="text-[20px] bg-blue-600 text-white rounded-md p-1 w-[40px] h-full flex justify-center items-center">
...
export async function getBase64FromUrl(url, autosign) {
  try {
    // Check if URL is already a base64 string
    if (url.startsWith("data:") || url.length > 1000) {
      // This is already a base64 string, return it directly
      if (autosign) {
        return url;
      } else {
        const suffixbase64 = url.includes(",") ? url.split(",").pop() : url;
        return suffixbase64;
      }
    }
...
export const signPdfFun = async (
  ...
  try {
    ...
    // Call the new API route instead of Parse cloud function
    const response = await fetch("/api/opensign/documents/sign", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    const resSignPdf = await response.json();
    if (resSignPdf) {
...
export const embedDocId = async (pdfOriginalWH, pdfDoc, documentId) => {
  ...
  // Use built-in font to avoid external fetch failures
...
export const contractDocument_old = async (documentId) => {
...
export const contractDocument = async (documentId) => {
  try {
    // console.log("Making API call to:", `/api/opensign/documents/${documentId}`);

    const res = await fetch(`/api/opensign/documents/${documentId}`, {
      cache: "no-store",
    });
    ...
    // API already returns an array, so return it directly
    return doc;
  } catch (err) {
    console.log("Err in documents/:id ", err);
...
export const getTenantDetails_old = async (objectId, contactId) => {
  try {
...
export const getTenantDetails = async (objectId, contactId) => {
  try {
    ...
    if (objectId) qs.set("userId", objectId);
    const res = await fetch(`/api/opensign/tenant?${qs.toString()}`, {
      cache: "no-store",
    });
    if (!res.ok) return "";
    const data = await res.json();
...
export const convertPdfArrayBuffer = async (url) => {
  try {
    if (!url) {
      console.error("No URL provided to convertPdfArrayBuffer");
      return "Error";
    }
...
export async function findContact_old(value) {
  try {
...
export async function findContact(value) {
  try {
    ...
    if (!res.ok) return [];
    const list = await res.json(); // [{ objectId, Name, Email, Phone? }]
    return list || [];
  } catch (error) {
    console.error("Error fetching suggestions:", error);
    return [];
...
export const convertBase64ToFile_old = async (pdfName, pdfBase64) => {
...
export const convertBase64ToFile = async (pdfName, pdfBase64) => {
  const fileName = sanitizeFileName(pdfName) + ".pdf";
  try {
    // Ensure pdfBase64 is a string before using string methods
    const base64String =
      typeof pdfBase64 === "string" ? pdfBase64 : String(pdfBase64);
    const payload = base64String.includes(",")
      ? base64String.split(",")[1]
      : base64String;
    const res = await fetch("/api/opensign/files", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ filename: fileName, base64: payload }),
    });
...
/**
 * Save PDF file in organized structure under document ID
 * @param {string} documentId - The document ID
 * @param {string} pdfName - The PDF filename
 * @param {string} pdfBase64 - The base64 encoded PDF
 * @param {string} fileType - The file type (versions, temp, final)
 * @returns {Promise<string>} The relative path to the saved file
 */
export const saveOrganizedPdfFile = async (
  documentId,
  pdfName,
  pdfBase64,
  fileType = "versions"
  ...
  try {
    ...
    const res = await fetch("/api/opensign/files/organized", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        documentId,
    ...
    console.log("error in saveOrganizedPdfFile", e);
    throw e;
  }
};
