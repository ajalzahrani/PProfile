...
import { createPerson, getPersonById, updatePerson } from "@/actions/persons";
...
export default function EditPersonPage({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const personResponse = await getPersonById(personId);

        if (personResponse.success) {
          form.reset({
            id: personResponse.person?.id,
            firstName: personResponse.person?.firstName ?? undefined,
            secondName: personResponse.person?.secondName ?? undefined,
            thirdName: personResponse.person?.thirdName ?? undefined,
            lastName: personResponse.person?.lastName ?? undefined,
            dob: personResponse.person?.dob ?? undefined,
            gender: personResponse.person?.gender as
              | "Male"
              | "Female"
              | undefined,
            jobTitleId: personResponse.person?.jobTitleId ?? undefined,
            unitId: personResponse.person?.unitId ?? undefined,
            rankId: personResponse.person?.rankId ?? undefined,
            sponsorId: personResponse.person?.sponsorId ?? undefined,
            nationalityId: personResponse.person?.nationalityId ?? undefined,
            citizenship: personResponse.person?.citizenship as
              | "Civilian"
              | "Foreigner"
              | "Other"
              | undefined,
            noriqama: personResponse.person?.noriqama ?? undefined,
            mrn: personResponse.person?.mrn ?? undefined,
            employeeNo: personResponse.person?.employeeNo ?? undefined,
            cardExpiryAt: personResponse.person?.cardExpiryAt ?? undefined,
            lastRenewalAt: personResponse.person?.lastRenewalAt ?? undefined,
            isActive: personResponse.person?.isActive ?? true,
            pictureLink: personResponse.person?.pictureLink ?? undefined,
          });
          // Set initial preview URL if there's an existing picture
          if (personResponse.person?.pictureLink) {
            setPreviewUrl(personResponse.person.pictureLink);
          }
        } else {
          toast({
            title: "Failed to load person",
            description: personResponse.error,
          });
        }
      } catch (error) {
  ...
  return (
    <PageShell>
      <PageHeader heading="Person Registration" text="Create a new person">
          ...
          <CardContent className="flex flex-col items-center">
            <div className="flex flex-col items-center gap-4">
              <Avatar className="h-32 w-32">
                <AvatarImage
                  src={previewUrl || getValues("pictureLink")}
